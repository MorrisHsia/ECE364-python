...
def loadphones():
    ...
    for line in content:
        ...
        name.append(lineAsList[0])
        phone.append(lineAsList[1])
...
def providersdic(filename):
    ...
    #consider making a for loop for this
    ...
    for file in dir:
        ...
        for sbc in sbcSet:
            if sbc in pdic:
                ...
                if pdic[sbc]<n:
                    price = pdic[sbc]
                    provider = file.strip('.dat')
                    #outputdic1.update({sbc:pdic[sbc]})

        minPrice = tuple([price, provider])
        outputdic.update({sbc: minPrice})
                #else:
